...
class Inception3(nn.Module):
    ...
    def __init__(self, num_classes=1000, aux_logits=True, transform_input=False, siamese=False, version='subtract'):
    ...
    def forward(self, x):
        if self.siamese:
            x0 = x[..., 0]
            x1 = x[..., 1]
            x0 = self.forward_pass(x0)
            x1 = self.forward_pass(x1)
            # N x 2048
            if self.version == 'cat':
                x = self.fc(torch.stack((x0, x1), -1))
            else:
