...
class MNASNet(torch.nn.Module):
    ...
    def __init__(self, alpha, num_classes=1000, dropout=0.2, siamese=False, version='subtract'):
        super(MNASNet, self).__init__()
        depths = _scale_depths([24, 40, 80, 96, 192, 320], alpha)
        self.siamese = siamese
        self.version = version
        arch = str(alpha).replace('.', '_')
        if len(arch) == 1:
            arch = '0_{}'.format(arch)
        self.arch = arch
        layers = [
    ...
    def forward(self, x):
        if self.siamese:
            ...
            x1 = x1.mean([2, 3])
            if self.version == 'cat':
                return self.classifier(torch.stack((x0, x1), -1))
            else:
                return self.classifier(torch.abs(x0 - x1))
        else:
            x = self.layers(x)
            # Equivalent to global avgpool and removing H and W dimensions.
            x = x.mean([2, 3])
...
def mnasnet1_0(siamese, version, pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    model = MNASNet(1.0, siamese=siamese, version=version, **kwargs)
    if pretrained:
